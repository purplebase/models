...
void main() {
  ...
  group('Replaceable Model Streaming Updates', () {
    test('query provider should update when replaceable model is updated via relay', () async {
      ...
      container.listen<StorageState<AppPack>>(
        ...
        (previous, next) {
          ...
          print('  Previous: ${previous?.models.map((m) => '${m.name} (${m.event.id.substring(0, 8)})').toList()}');
          print('  Next: ${next.models.map((m) => '${m.name} (${m.event.id.substring(0, 8)})').toList()}');
          print('  Previous == Next: ${previous == next}');
          print('  Previous?.props: ${previous?.props.map((p) => p is List ? '${(p as List).length} items' : p)}');
          print('  Next.props: ${next.props.map((p) => p is List ? '${(p as List).length} items' : p)}');
      ...
      print('Event IDs: ${state1.models.map((m) => m.event.id.substring(0, 8)).toList()}');
      ...
      print('Event IDs: ${state2.models.map((m) => m.event.id.substring(0, 8)).toList()}');
      
      print('\n=== Summary ===');
      print('Total notifications: $notificationCount');
      print('Captured names: $capturedNames');
      print('Captured event IDs: ${capturedEventIds.map((id) => id.substring(0, 8)).toList()}');
      ...
      expect(state2.models, hasLength(1),
          reason: 'Should have exactly 1 model (replaced, not added)');
      expect(state2.models.first.name, equals('Version 2 - Updated'),
          reason: 'Should have the updated name');
      expect(state2.models.first.event.id, equals(pack2.event.id),
          reason: 'Should have the new event ID');

      // Verify we captured the update
      expect(capturedNames, contains('Version 2 - Updated'),
    ...
    test('verify relay subscription properly replaces old events', () async {
      ...
      print('Names: ${state1.events.map((e) => e['tags'].firstWhere((t) => t[0] == 'name', orElse: () => ['', 'none'])[1]).toList()}');
      print('Event IDs: ${state1.events.map((e) => (e['id'] as String).substring(0, 8)).toList()}');
      ...
      print('Names: ${state2.events.map((e) => e['tags'].firstWhere((t) => t[0] == 'name', orElse: () => ['', 'none'])[1]).toList()}');
      print('Event IDs: ${state2.events.map((e) => (e['id'] as String).substring(0, 8)).toList()}');

      // The relay subscription should have replaced the old event
      expect(state2.events, hasLength(1),
          reason: 'Should still have 1 event (replaced, not added)');
      
      final finalName = state2.events.first['tags']
          .firstWhere((t) => t[0] == 'name', orElse: () => ['', 'none'])[1];
      expect(finalName, equals('Updated'),
          reason: 'Should have the updated name');
    });
  });
}
