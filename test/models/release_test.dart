...
void main() {
  ...
  group('Release', () {
    ...
    test('release <-> file metadata relationship', () async {
      ...
      final allReleases = await storage.query(Request<Release>([
        RequestFilter<Release>(kinds: {30063})
      ]));
      final allFiles = await storage.query(Request<FileMetadata>([
        RequestFilter<FileMetadata>(kinds: {1063})
      ]));
      final loadedRelease = allReleases.firstWhere((r) => r.id == release.id);
      final loadedFileMetadata =
          allFiles.firstWhere((f) => f.id == fileMetadata.id);

      // Check that the Release lists the FileMetadata
      expect(
          loadedRelease.fileMetadatas.toList(), contains(loadedFileMetadata));
